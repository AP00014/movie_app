...
const TechBadge = ({ active, children, onClick }: any) => (
    <div onClick={onClick} 
         style={{
             ...
             background: active ? 'linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%)' : '#f1f5f9',
             color: active ? 'white' : '#64748b', transition:'all 0.2s ease',
             border: active ? 'none' : '1px solid #e2e8f0',
             boxShadow: active ? '0 4px 12px rgba(109, 40, 217, 0.2)' : 'none'
         }}>
        {children}
    </div>
);
...
export default function NewAnimationPage() {
  ...
  const handleSubmit = async (e: any) => {
      ...
      console.log("Submitting Animation Project:", formData);
      setTimeout(() => {
          setIsSubmitting(false);
          router.push('/admin/content/animation');
      }, 1500);
  };
       ...
       <main className="admin-main">
            ...
            <header className="admin-header">
               <div style={{display:'flex', alignItems:'center', gap:'15px'}}>
                   <Link href="/admin/content/animation" style={{display:'flex', alignItems:'center', justifyContent:'center', width:'36px', height:'36px', borderRadius:'10px', background:'white', border:'1px solid #e2e8f0', color:'#64748b', transition:'all 0.2s'}}>
                        <ChevronLeft size={20} />
                   </Link>
                   <div>
                       <div className="page-title" style={{fontSize:'20px'}}>New Animation Project</div>
                       <div style={{fontSize:'12px', color:'#94a3b8', fontWeight:500}}>Create a world of imagination</div>
                   </div>
               </div>
               
               <div className="header-actions">
                   <button onClick={handleSubmit} disabled={isSubmitting} className="header-btn" 
                           style={{width:'auto', padding:'0 25px', background: isSubmitting ? '#94a3b8' : 'linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%)', color:'white', border:'none', borderRadius:'10px', fontWeight:600, gap:'8px', boxShadow:'0 4px 12px rgba(109, 40, 217, 0.3)'}}>
                       {isSubmitting ? 'Processing...' : <><Save size={18} /> Publish Animation</>}
                   </button>
               </div>
           </header>
                   ...
                   {activeTab === 'overview' && (
                       <div className="pro-grid-2-1">
                           <div style={{display:'flex', flexDirection:'column', gap:'25px'}}>
                               <ProSection icon={Film} title="Animation Identity">
                                   <div className="form-group">
                                       <Label>Title</Label>
                                       <Input name="title" value={formData.title} onChange={handleChange} placeholder="e.g. Spirited Away" required icon={Type} />
                                   </div>
                                   <div className="form-group">
                                       <Label>Tagline</Label>
                                       <Input name="tagline" value={formData.tagline} onChange={handleChange} placeholder="e.g. A journey into the unknown" icon={MessageSquareQuote} />
                                   </div>
                                   <div className="form-group">
                           ...
                           <div style={{display:'flex', flexDirection:'column', gap:'25px'}}>
                               <ProSection icon={Globe} title="Classification">
                                    <div className="form-group">
                                       ...
                                       <Input name="genres" value={formData.genres} onChange={handleChange} placeholder="Fantasy, Adventure" icon={Hash} />
                                   </div>
                                   <div className="pro-grid-2">
                                       <div className="form-group">
                                           <Label>Moods (Comma separated)</Label>
                                           <Input name="moods" value={formData.moods} onChange={handleChange} placeholder="Whimsical, Magical" icon={Sparkles} />
                                       </div>
                                       <div className="form-group">
                                   ...
                                   <div className="pro-grid-2">
                                       ...
                                       <div>
                                           ...
                                           <Input name="duration" value={formData.duration} onChange={handleChange} placeholder="1h 45m" icon={Clock} />
                                       </div>
                                   </div>
                                   <div className="pro-grid-2">
                                        <div>
                           ...
                           <ProSection icon={Mic} title="Technical Specs">
                               <div className="pro-grid-2" style={{gap:'20px'}}>
                                   ...
                                   <div className="form-group">
                                       ...
                                       <Input name="audioLanguages" value={formData.audioLanguages} onChange={handleChange} placeholder="English, Japanese" icon={Globe} />
                                   </div>
                                   <div className="form-group">
                       ...
                       <ProSection icon={Users} title="Cast & Crew">
                           <div className="form-group">
                               <Label>Director</Label>
                                <Input name="director" value={formData.director} onChange={handleChange} placeholder="Hayao Miyazaki" icon={User} />
                           </div>
                           <div className="pro-grid-2">
                               <div className="form-group">
                                   <Label>Writers</Label>
                           ...
                           <div style={{marginTop:'20px', paddingTop:'20px', borderTop:'1px solid #e2e8f0'}}>
                               ...
                               <div style={{display:'flex', gap:'15px', alignItems:'flex-end', marginBottom:'20px', background:'#f8fafc', padding:'15px', borderRadius:'12px'}}>
                                   <div style={{flex:1}}>
                                       ...
                                       <Input value={newCast.name} onChange={(e:any) => setNewCast({...newCast, name: e.target.value})} placeholder="e.g. Rumi Hiiragi" style={{background:'white'}} icon={User} />
                                   </div>
                                   <div style={{flex:1}}>
                                       <label style={{fontSize:'11px', fontWeight:600, color:'#64748b', marginBottom:'4px', display:'block'}}>Character / Role</label>
                                       <Input value={newCast.role} onChange={(e:any) => setNewCast({...newCast, role: e.target.value})} placeholder="e.g. Chihiro" style={{background:'white'}} icon={UserCog} />
                                   </div>
                                   <div style={{flex:1}}>
                                   ...
                                   <button type="button" onClick={addCastMember} style={{background:'#8b5cf6', color:'white', border:'none', borderRadius:'10px', width:'42px', height:'42px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer'}}>
                                       <CheckCircle size={20} />
                                   </button>
                               </div>
                       ...
                       <ProSection icon={Globe} title="Distribution-Channels">
                           <div className="pro-grid-2">
                               <div className="section-card" onClick={() => handleSectionToggle('hero')} 
                                    style={{padding:'20px', border: formData.sections.hero ? '2px solid #8b5cf6' : '1px solid #e2e8f0', background: formData.sections.hero ? '#f3e8ff' : 'white', borderRadius:'12px', cursor:'pointer'}}>
                                   <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
                                        <Label>Home Hero Carousel</Label>
                                        <div style={{width:'20px', height:'20px', borderRadius:'50%', border: formData.sections.hero?'none':'2px solid #cbd5e1', background:formData.sections.hero?'#8b5cf6':'none', display:'flex', alignItems:'center', justifyContent:'center'}}>
                                            {formData.sections.hero && <CheckCircle size={14} color="white" />}
                                        </div>
                                   </div>
                                   <p style={{fontSize:'12px', color:'#64748b'}}>Pin this animation to the main landing slider.</p>
                               </div>
                               <div className="section-card" onClick={() => handleSectionToggle('trending')} 
                                    style={{padding:'20px', border: formData.sections.trending ? '2px solid #8b5cf6' : '1px solid #e2e8f0', background: formData.sections.trending ? '#f3e8ff' : 'white', borderRadius:'12px', cursor:'pointer'}}>
                                   <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
                                        <Label>Trending Now</Label>
                                        <div style={{width:'20px', height:'20px', borderRadius:'50%', border: formData.sections.trending?'none':'2px solid #cbd5e1', background:formData.sections.trending?'#8b5cf6':'none', display:'flex', alignItems:'center', justifyContent:'center'}}>
                               ...
                               <div className="section-card" onClick={() => handleSectionToggle('newArrival')} 
                                    style={{padding:'20px', border: formData.sections.newArrival ? '2px solid #8b5cf6' : '1px solid #e2e8f0', background: formData.sections.newArrival ? '#f3e8ff' : 'white', borderRadius:'12px', cursor:'pointer'}}>
                                   <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
                                        <Label>New Releases</Label>
                                        <div style={{width:'20px', height:'20px', borderRadius:'50%', border: formData.sections.newArrival?'none':'2px solid #cbd5e1', background:formData.sections.newArrival?'#8b5cf6':'none', display:'flex', alignItems:'center', justifyContent:'center'}}>
                               ...
                               <div className="section-card" onClick={() => handleSectionToggle('editorsChoice')} 
                                    style={{padding:'20px', border: formData.sections.editorsChoice ? '2px solid #8b5cf6' : '1px solid #e2e8f0', background: formData.sections.editorsChoice ? '#f3e8ff' : 'white', borderRadius:'12px', cursor:'pointer'}}>
                                   <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
                                        <Label>Editor's Choice</Label>
                                        <div style={{width:'20px', height:'20px', borderRadius:'50%', border: formData.sections.editorsChoice?'none':'2px solid #cbd5e1', background:formData.sections.editorsChoice?'#8b5cf6':'none', display:'flex', alignItems:'center', justifyContent:'center'}}>
                           ...
                           <div style={{marginTop:'30px', paddingTop:'30px', borderTop:'1px solid #e2e8f0'}}>
                               ...
                               <div style={{display:'flex', gap:'10px', marginBottom:'15px'}}>
                                   <Input placeholder="e.g. Studio Ghibli, Anime"  id="collectionInput" 
                                          onKeyDown={(e: any) => {
                                              if (e.key === 'Enter') {
                                                  e.preventDefault();
                               ...
                               <div style={{display:'flex', flexWrap:'wrap', gap:'10px'}}>
                                   {formData.customCollections?.map((col: string) => (
                                       <span key={col} style={{display:'flex', alignItems:'center', gap:'6px', padding:'6px 14px', background:'#f3e8ff', color:'#8b5cf6', borderRadius:'20px', fontSize:'13px', fontWeight:600}}>
                                           {col}
                                           <button type="button" onClick={() => setFormData(prev => ({...prev, customCollections: prev.customCollections.filter((c: string) => c !== col)}))} 
                                                   style={{background:'none', border:'none', cursor:'pointer', color:'#8b5cf6', display:'flex', alignItems:'center'}}>
                                               ×
                                           </button>
                                       </span>
                                   ))}
...
const ProSection = ({icon: Icon, title, children}: any) => (
    <section style={{background:'white', padding:'30px', borderRadius:'16px', border:'1px solid #e2e8f0', boxShadow:'0 2px 10px rgba(0,0,0,0.02)'}}>
        <h3 style={{marginTop:0, marginBottom:'25px', fontSize:'16px', fontWeight:700, display:'flex', alignItems:'center', gap:'10px', color:'#0f172a'}}>
            <div style={{padding:'8px', background:'#f1f5f9', borderRadius:'8px', color:'#8b5cf6'}}><Icon size={18} /></div>
            {title}
        </h3>
        {children}
    </section>
